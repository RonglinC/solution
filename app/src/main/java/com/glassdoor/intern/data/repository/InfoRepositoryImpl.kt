...
internal class InfoRepositoryImpl @Inject constructor(
    ...
    override suspend fun getHeaderInfo(): Result<HeaderInfo, Throwable> =
        try {
            val infoDto = infoApi.getInfo()
            with(infoDto) {
                when {
                    header != null -> {
                        val headerInfo = headerInfoMapper.toDomain(header,items)
                        //DONE("Convert DTO into domain model")
                        Ok(headerInfo)
                    }
                    //Done("Convert to error")
                    error != null -> Err(Exception("Error from API"))
                    // DONE("Convert to error")
                    else -> Err(Exception("Error occurred"))
                }
            }
